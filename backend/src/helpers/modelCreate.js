...
class ModelCreate {
    ...
    findById (_id, columns = this.selectableProps, orderBy = ORDER_BY) {
        return this.knex.select(columns).from(this.tableName).where({_id}).orderBy(orderBy).timeout(this.timeout);
    }
    ...
    deleteOne (_id) {
        ...
        return this.knex.update({
            deleted: true,
            deletedAt: new Date()
        }).from(this.tableName).where({_id}).timeout(this.timeout);
    }
        ...
        if (isArray(ids) && String instanceof head(ids)) {
            if (this.transaction) {
                return this.transaction(this.tableName).update({
                    deleted: true,
                    deletedat: new Date()
                }).whereIn('_id', ids).timeout(this.timeout);
            }
            return this.knex.update({
                deleted: true,
                deletedat: new Date()
            }).from(this.tableName).whereIn('_id', ids).timeout(this.timeout);
        }
    }

    async countDocuments (filters = {}) {
        const {count} = head(await this.knex(this.tableName).count('_id').where(filters).timeout(this.timeout));
        return count;
    }
